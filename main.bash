...
touch withtime.csv #Time catch
touch timesteam.csv #Time catch
...
# Ready to make .csv output      
       ...
       sed -i "s|^[^,]*|`date '+%d/%m/%Y %H:%M'`|g" Finallogtranend1.csv > timesteam.csv
      ...
      awk -F '|' '!x[$1,$2,$3,$4]++' Finallogdone.csv > filename_dedup.csv
           sed -i "s|^[^,]*|`date '+%d/%m/%Y %H:%M'`|g" filename_dedup.csv > withtime.csv
      cat withtime.csv |  awk -F"[,]+" '{print $1,$5,$6,$8}' >> WebLog.csv
      cat withspace.csv |  awk -F"[,]+" '{print $1,$2,$4,$8}' >> WebLog.csv
